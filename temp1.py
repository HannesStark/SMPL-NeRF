...
for i, ray_translation in enumerate(rays_translations):
...
sm1 = trimesh.creation.uv_sphere(radius=0.01)
...
sm_warped_ray = trimesh.creation.uv_sphere(radius=0.01)
sm_warped_ray.visual.vertex_colors = [0, 1.0, 1.0]
tfs1 = np.tile(np.eye(4), (len(all_warped_samples), 1, 1))
tfs1[:, :3, 3] = all_warped_samples
warped_ray = pyrender.Mesh.from_trimesh(sm_warped_ray, poses=tfs1)
